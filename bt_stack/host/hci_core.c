...
#if (defined(CONFIG_BT_OBSERVER) && defined(CONFIG_BT_EXT_ADV)) || defined(CONFIG_BT_USER_PHY_UPDATE)
                        ...
                        const struct bt_data *sd, size_t sd_len)
{
    ...
    adv->id = param->id;
    err = le_ext_adv_param_set(adv, param, sd || (param->options & BT_LE_ADV_OPT_USE_NAME));
    if (err) {
        return err;
    }
                        ...
                        struct bt_le_ext_adv_start_param *param)
{
    ...
    atomic_set_bit_to(adv->flags, BT_ADV_LIMITED, param && (param->timeout > 0 || param->num_events > 0));
