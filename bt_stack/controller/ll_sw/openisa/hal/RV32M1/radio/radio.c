...
#define MAN_DSM_ON (RSIM_DSM_CONTROL_MAN_DEEP_SLEEP_STATUS_MASK | RSIM_DSM_CONTROL_DSM_MAN_READY_MASK | RSIM_DSM_CONTROL_MAN_SLEEP_REQUEST_MASK)
...
static uint8_t MALIGN(4) _pkt_scratch[((RADIO_PDU_LEN_MAX + 3) > PDU_AC_LL_SIZE_MAX) ?
                                          (RADIO_PDU_LEN_MAX + 3) :
                                          PDU_AC_LL_SIZE_MAX];
...
void radio_setup(void)
{
    ...
    rx_wu = (XCVR_TSM->END_OF_SEQ & XCVR_TSM_END_OF_SEQ_END_OF_RX_WU_MASK) >> XCVR_TSM_END_OF_SEQ_END_OF_RX_WU_SHIFT;
    tx_wu = (XCVR_TSM->END_OF_SEQ & XCVR_TSM_END_OF_SEQ_END_OF_TX_WU_MASK) >> XCVR_TSM_END_OF_SEQ_END_OF_TX_WU_SHIFT;
...
void radio_aa_set(uint8_t *aa)
{
    ...
    GENFSK->NTW_ADR_CTRL |= (uint32_t)((1 << 0) << GENFSK_NTW_ADR_CTRL_NTW_ADR_EN_SHIFT);
