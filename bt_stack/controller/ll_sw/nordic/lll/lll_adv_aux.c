...
static int prepare_cb(struct lll_prepare_param *p)
{
    ...
    if (lll_preempt_calc(evt, (TICKER_ID_ADV_AUX_BASE + ull_adv_aux_lll_handle_get(lll)),
                         ticks_at_event)) {
                             ...
                             uint8_t rssi_ready)
{
    ...
    if ((pdu_rx->type == PDU_ADV_TYPE_AUX_SCAN_REQ) &&
        ...
        memcpy(rx->pdu, pdu_rx, (offsetof(struct pdu_adv, connect_ind) + sizeof(struct pdu_adv_connect_ind)));
        ftr = &(rx->hdr.rx_ftr);
        ftr->param = lll;
