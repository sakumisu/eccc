...
static int prepare_cb(struct lll_prepare_param *p)
{
    ...
    if (lll_preempt_calc(evt, (TICKER_ID_SCAN_BASE + ull_scan_lll_handle_get(lll)),
                         ticks_at_event)) {
                             ...
                             uint8_t rl_idx, uint8_t rssi_ready)
{
    ...
    if (0) {
        ...
        memcpy(rx->pdu, pdu_tx, (offsetof(struct pdu_adv, connect_ind) + sizeof(struct pdu_adv_connect_ind)));
