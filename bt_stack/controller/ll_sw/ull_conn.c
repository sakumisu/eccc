                                       ...
                                       void (*fp_mfy_select_or_use)(void *))
{
...

    {
        ...
        conn->llcp.conn_upd.pdu_win_offset = (uint16_t *)&pdu_ctrl_tx->llctrl.conn_update_ind.win_offset;
                                     ...
                                     uint16_t event_counter)
{
    ...
    } else if (((event_counter - conn->llcp.chan_map.instant) & 0xFFFF) <= 0x7FFF) {
        struct lll_conn *lll = &conn->lll;
		 ...
		 */
            ...
            if (!conn->lll.role) {
                if ((conn->llcp_conn_param.req !=
                    ...
                    struct pdu_data_llctrl_conn_param_req *cpr = (void *)&pdu_rx->llctrl.conn_param_req;
                    struct lll_conn *lll = &conn->lll;
