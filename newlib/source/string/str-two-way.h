	 ...
	 of the period.  */
        ...
        while (AVAILABLE(haystack, haystack_len, j, needle_len)) {
            ...
            if (needle_len <= i) {
                ...
                i = suffix - 1;
                while (memory < i + 1 && (CANON_ELEMENT(needle[i]) == CANON_ELEMENT(haystack[i + j])))
                    --i;
                if (i + 1 < memory + 1)
	 ...
	 required, and any mismatch results in a maximal shift.  */
        ...
        while (AVAILABLE(haystack, haystack_len, j, needle_len)) {
            ...
            if (needle_len <= i) {
                /* Scan for matches in left half.  */
                i = suffix - 1;
                while (i != SIZE_MAX && (CANON_ELEMENT(needle[i]) == CANON_ELEMENT(haystack[i + j])))
                    --i;
                if (i == SIZE_MAX)
                    return (RETURN_TYPE)(haystack + j);
                j += period;
	 ...
	 of the period.  */
        ...
        while (AVAILABLE(haystack, haystack_len, j, needle_len)) {
	     ...
	     already been matched, by virtue of the shift table.  */
            i = MAX(suffix, memory);
            while (i < needle_len - 1 && (CANON_ELEMENT(needle[i]) == CANON_ELEMENT(haystack[i + j])))
                ++i;
            if (needle_len - 1 <= i) {
                /* Scan for matches in left half.  */
                i = suffix - 1;
